defmodule Producer.Publisher do
  use GenServer
  alias :jsx, as: JSX
  @routing_key "routing_key_gr"
  @exchange "exchange_gr"
  @queue "queue_gr"
  ...
  def handle_info({:init, queue, exchange, routing_key}, _state) do
    ...
    # In the context of a cluster, the below options allow the cluster
    # to have redundant copies across multiple servers.
    options = [arguments: [{'x-ha-policy', :longstr, "nodes"},
                           {'x-ha-nodes',  :array, ["rabbit@lenovo-legion"]}]]
