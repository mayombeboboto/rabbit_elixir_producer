defmodule Producer do
  ...
  def handle_cast({:publish, msg}, state) do
    AMQP.Basic.publish(state.chan, @exchange1, "Unknown", msg)
    {:noreply, state}
  end
  ...
  def handle_info(:init, _state) do
    ...
    AMQP.Exchange.declare(chan, @exchange0, :fanout)

    # A message sent to this exchange would be routed to @exchange0
    # In case of incorrect route.
    # AMQP.Exchange.declare(chan, @exchange1, :direct, arguments: {'alternate-exchange', :longstr, @exchange0})
    options = [arguments: [{'alternate-exchange', :longstr, @exchange0}]]
    AMQP.Exchange.declare(chan, @exchange1, :topic, options)
